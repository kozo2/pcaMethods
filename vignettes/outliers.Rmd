---
title: "Handling of data containing outliers"
author:
...
a robust analysis method. Hereto we provide `robustSvd`, a singular value
decomposition robust to outliers. `robustPca` is a PCA implementation that
resembles the original `R` `prcomp` method, with the difference
that it uses `robustSvd` instead of the standard `svd` function.

Robust SVD and its application to microarray data were proposed in [@hawkins01]
and [@liu03]. The algorithm is based on the idea to use a sequential estimation
...
Now we use `PPCA` to estimate the PCA solution,
but set the outliers NA before.

```{r runPpca, results='hide'}
mdcNa        <- mdc
mdcNa[cond]  <- NA
...
results to the optimal PCA solution for the complete data set (which is `resSvd`).
The figure below plots the estimated and original loadings against each other.

```{r svdPlot, fig.width=8, fig.height=8, echo=FALSE, results='hide', fig.cap="Figures show (from left to right): Original PCA solution vs. solution on data with outliers; original PCA solution vs. robust PCA solution on data with outliers; original PCA solution vs. PPCA solution on data where outliers=NA; robust PCA solution vs. PPCA solution on data with outliers / outliers=NA."}
par(mfrow=c(2,2))
